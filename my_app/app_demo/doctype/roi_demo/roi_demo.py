...
class ROI_Demo(Document):
    ...
    def process_roi(self):
        known_rois = frappe.get_all(
            "ROI_Demo", filters={"person": ("!=", "")}, fields=["person", "encoding"]
        )
        if known_rois:
...
def process_labelled_photos():
    ...
    unrecognized_rois = frappe.get_all(
        "ROI_Demo", filters={"person": ""}, fields=["name", "encoding"]
    )
    recognized_photos = {}
    ...
    if known_rois:
        ...
        for roi, person in recognized_photos.items():
            frappe.db.set_value("ROI_Demo", roi, "person", person)
...
def process_unlabelled_photos(unrecognized_rois=None):
    ...
    person = frappe.new_doc("Person_Demo")
    person.person_name = frappe.mock("name")
    person.save()

    frappe.db.set_value("ROI_Demo", roi.name, "person", person.name)
