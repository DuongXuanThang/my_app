...
class Assets(Document):
    def before_save(self):
        # Check exist Asset
        asset_exists = frappe.get_list("Assets", filters={"name": self.name}, limit=1)
        if asset_exists:
            self.update_asset()
        else:
            self.check_and_add_asset()
    ...
    def update_asset(self):
        ...
        if custom_field is None:
            ...
            if product_id_ai is None:
                ...
                products.update_by_id(collection_name, product_id_ai, self.asset_name)
                imageSourceDBs = frappe.get_all("Assets_Image", filters={"parent": self.name}, fields=["custom_field", "name", "uri_image"])
                for index, photo in enumerate(self.photos):
                    if not hasattr(photo, "custom_field") or photo.custom_field is None:
                        ...
                        response = products.add(collection_name, product_id_ai, self.asset_name, image_ids, image_paths)
    ...
    def check_and_add_asset(self):
        RECOGNITION_API_KEY: str = '00000000-0000-0000-0000-000000000002'
        deep_vision: DeepVision = DeepVision()
        product_recognition: ProductRecognitionService = deep_vision.init_product_recognition_service(RECOGNITION_API_KEY)
        product_name = self.asset_name
        base_url = frappe.utils.get_request_site_address()
