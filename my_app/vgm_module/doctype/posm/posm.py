...
class POSM(Document):
    def before_save(self):
        # Check exist POSM
        posm_exists = frappe.get_list("POSM", filters={"name": self.name}, limit=1)
        if posm_exists:
            self.update_posm()
        else:
            self.check_and_add_posm()

    def update_posm(self):
        ...
        if custom_field is None:
            ...
            if product_id_ai is None:
                ...
                products.update_by_id(collection_name, product_id_ai, self.posm_name)
                imageSourceDBs = frappe.get_all("POSM_Image", filters={"parent": self.name}, fields=["custom_field", "name", "uri_image"])
                for index, photo in enumerate(self.photos):
                    if not hasattr(photo, "custom_field") or photo.custom_field is None:
                        ...
                        response = products.add(collection_name, product_id_ai, self.posm_name, image_ids, image_paths)
    ...
    def check_and_add_posm(self):
        RECOGNITION_API_KEY: str = '00000000-0000-0000-0000-000000000002'
        deep_vision: DeepVision = DeepVision()
        product_recognition: ProductRecognitionService = deep_vision.init_product_recognition_service(RECOGNITION_API_KEY)
        product_name = self.posm_name
        base_url = frappe.utils.get_request_site_address()
